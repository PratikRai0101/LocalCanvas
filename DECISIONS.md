...
Decision: Write each local recording atomically to `.localcanvas/voice-notes/<drawing-UUID>/<note-UUID>.<format>`, using the durable drawing metadata element already present in LocalCanvas scenes. Store the note UUID, MIME type, duration, optional local transcript, and optional attachment target in an ordinary Excalidraw marker element's `customData.localcanvas` payload. The marker is portable; playback gracefully reports a missing local recording when the drawing is opened elsewhere. New recordings prefer M4A so macOS Speech can transcribe them with `requiresOnDeviceRecognition` and never fall back to a network recognizer.
Consequences: Audio and transcription stay local and do not compromise `.excalidraw` compatibility. Moves and renames preserve new audio attachments because their storage key is the durable drawing UUID. Legacy path-hash recordings remain readable and deletable at their original path, but cannot follow a later move.
